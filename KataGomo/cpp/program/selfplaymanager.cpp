...
void SelfplayManager::runDataWriteLoopImpl(ModelData* modelData) {
  ...
  while(true) {
    ...
    if (gameData->hitTurnLimit) {
      if (logger != NULL) {
        logger->write("Discarding game because it hit the turn limit (maxMovesPerGame).");
      }
      delete gameData;
      continue;
    }
    ...
    modelData->gameFinishedCount += 1;
    if (logger != NULL && modelData->gameFinishedCount % logGamesEvery == 0) {
      logger->write("Finished " + Global::int64ToString(modelData->gameFinishedCount) + " games with " + modelData->modelName);
    }
