...
void NNInputs::fillRowV7(
  ...
  for(int y = 0; y<ySize; y++) {
    for(int x = 0; x<xSize; x++) {
      ...
      if(stone != C_EMPTY && stone != C_WALL) {
        Player p = getPiecePla(stone);
        Color pt = getPieceType(stone);
        if(pt >= 1 && pt <= 11) {
          int idx = (p == pla) ? (0 + pt) : (11 + pt);
  ...
  } else if(board.stage == 1)  // place
  {
    ...
    if(!board.isOnBoard(chosenMove)) {
      ...
      setRowBin(rowBin, pos, 23, 1.0f, posStride, featureStride);
    }
  } else
    ASSERT_UNREACHABLE;
