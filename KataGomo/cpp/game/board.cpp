...
void Board::init(int xS, int yS)
{
  ...
  if (x_size >= 13 && y_size >= 13) {
      ...
      for (int y = 0; y < 13; y++) {
          for (int x = 0; x < 13; x++) {
              char c = layout[y * 13 + x];
              if (c == '.') continue;
              Player p = (c >= 'A' && c <= 'Z') ? P_BLACK : P_WHITE;
              char lower = (c >= 'A' && c <= 'Z') ? (c - 'A' + 'a') : c;
              Color pt = PT_NONE;
              switch(lower) {
                  case 'a': pt = PT_ROOK; break;
                  case 'b': pt = PT_KNIGHT; break;
                  case 'c': pt = PT_ELEPHANT; break;
                  case 'd': pt = PT_ADVISOR; break;
                  case 'e': pt = PT_KING; break;
                  case 'f': pt = PT_CANNON; break;
                  case 'g': pt = PT_PAWN; break;
                  case 'h': pt = PT_LEI; break;
                  case 'i': pt = PT_FENG; break;
                  case 'j': pt = PT_WEI; break;
              }
              if (pt != PT_NONE) {
                  setStone(Location::getLoc(x, y, x_size), makePiece(p, pt));
              }
          }
      }
  }
...
char PlayerIO::colorToChar(Color c)
{
  ...
  char base = '?';
  switch(pt) {
    case PT_ROOK:     base = 'a'; break;
    case PT_KNIGHT:   base = 'b'; break;
    case PT_ELEPHANT: base = 'c'; break;
    case PT_ADVISOR:  base = 'd'; break;
    case PT_KING:     base = 'e'; break;
    case PT_CANNON:   base = 'f'; break;
    case PT_PAWN:     base = 'g'; break;
    case PT_LEI:      base = 'h'; break;
    case PT_FENG:     base = 'i'; break;
    case PT_WEI:      base = 'j'; break;
  }
  if(p == P_BLACK) return toupper(base);
  return base;
}

string PlayerIO::playerToString(Color c)
{
  if (c == P_BLACK) return "Black";
  if (c == P_WHITE) return "White";
  if (c == C_EMPTY) return "Empty";
  return "Wall";
}

string PlayerIO::playerToStringShort(Color c)
{
  if (c == P_BLACK) return "B";
  if (c == P_WHITE) return "W";
  if (c == C_EMPTY) return "E";
  return "";
}
