...
class Model(torch.nn.Module):
    def __init__(self, config: modelconfigs.ModelConfig, pos_len: int):
        ...
        if config["initial_conv_1x1"]:
            self.conv_spatial = torch.nn.Conv2d(25, self.c_trunk, kernel_size=1, padding="same", bias=False)
        else:
            self.conv_spatial = torch.nn.Conv2d(25, self.c_trunk, kernel_size=3, padding="same", bias=False)
        self.linear_global = torch.nn.Linear(19, self.c_trunk, bias=False)

        self.bin_input_shape = [25, pos_len, pos_len]
        self.global_input_shape = [19]

        self.blocks = torch.nn.ModuleList()
        for i, block_config in enumerate(self.block_kind):
            ...
            # Now set it explicitly so ONNX tracer sees it
            setattr(self, f"fixed_block_{i}", self.blocks[-1])
    ...
    def forward(self, input_spatial, input_global):
        ...
        if self.has_intermediate_head:
            ...
            for i in range(len(self.blocks)):
                block = getattr(self, f"fixed_block_{i}")
                out = block(out, mask=mask, mask_sum_hw=mask_sum_hw, mask_sum=mask_sum)
                count += 1
